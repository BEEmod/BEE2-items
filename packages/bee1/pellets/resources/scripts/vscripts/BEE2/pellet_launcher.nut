...
// infinite <- false; // Is the pellet lifetime infinite
...
function pellet_launched() {
	...
	while(1) {
		...
		cur_pellet = pellet;
		print(cur_pellet.GetClassname)
		EntFireByHandle(cur_pellet, "AddOutput", "OnUser1 " + self.GetName() + ":CallScriptFunction:kill_pellet::-1", 0.0, self, self);
		break;
	}
...
function turn_on() {
	...
	if (infinite) {
		...
		EntFireByHandle(self, "Enable", "", 0.0, self, self);
	}
}
...
self.ConnectOutput("OnBallCaught", "pellet_launched")